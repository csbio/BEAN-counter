...
def main(dataset, dataset_file, table, val_name, strain_table_f, strain_columns, sample_table_f, condition_columns, output_file, verbosity):
    ...
    # If the table boolean is True, then reshape into a table and write everything out as one table
        ...
        if strain_table_f is not None and strain_columns is not None:
            strain_table = read_sample_table(strain_table_f)
            strain_table_idx = {(strain_table.loc[i, 'Strain_ID'], strain_table.loc[i, 'Barcode']):i for i in range(strain_table.shape[0])}
        ...
        if val_name is None:
            val_name = 'value'
        default_cols = ['Strain_ID', 'Barcode', 'screen_name', 'expt_id', val_name]
        extra_strain_cols = []
        ...
        for i in range(matrix.shape[0]):
            ...
            for j in range(matrix.shape[1]):
                ...
                final_data[2].append(conditions[j][0])
                final_data[3].append(conditions[j][1])
                final_data[4].append(matrix[i, j])
...
if __name__ == '__main__':
    ...
    main(dataset, dataset_file, table, val_name, strain_table_f, strain_columns, sample_table_f, condition_columns, args.output_file, verbosity)
