...
parser.add_argument('dataset_file', help = 'The dataset to cluster. If --new_dataset is specified, the clustering will be based on these data, but the actual data themselves will come from the --new_dataset file.')
...
parser.add_argument('--new_dataset', help = 'A dataset object (strains, conditions, matrix), saved to file, from which the data, but not the clustering, if the output will come. The dataset must have the same rows and columns, and in the same order, as the data in the "dataset" argument')
parser.add_argument('-v', '--verbosity', help = 'The level of verbosity printed to stdout. Ranges from 0 to 3, 1 is default.')
args = parser.parse_args()

# Load in dataset
dataset_filename = os.path.abspath(args.dataset_file)
dataset_f = gzip.open(dataset_filename)
...
if args.new_dataset is not None:
    ...
    new_dataset_f = gzip.open(new_dataset_filename)
...
output_folder = os.path.splitext(os.path.splitext(dataset_filename)[0])[0]
